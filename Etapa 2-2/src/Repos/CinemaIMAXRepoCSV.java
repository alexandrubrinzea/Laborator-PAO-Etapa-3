...
public class CinemaIMAXRepoCSV  {
    private ArrayList<CinemaIMAX> cinemaimax = new ArrayList<CinemaIMAX>();
    private static int counter = 0;
    public CinemaIMAXRepoCSV(String fileName) {
        try {
            ...
            while (scanner.hasNext()) {
                ...
                try {
                    cinemaimax.add(new CinemaIMAX(values[0], values[1], new Integer(values[2]), values[3], values[4], values[5], values[6], new Integer(values[7]), values[8], new Venue(values[9], new Integer(values[10]), new Boolean(values[11]), new Boolean(values[12]))));
